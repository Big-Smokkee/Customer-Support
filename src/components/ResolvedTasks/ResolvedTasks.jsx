...
const ResolvedTasks = ({ resolveTaskNumber }) => {
    return (
        <div className='border border-gray-200 shadow-xl p-3'>
            ...
            {
                resolveTaskNumber.length === 0 && <p className='text-gray-500'>No resolved tasks yet.</p>
            }
            {
                resolveTaskNumber.length !== 0 && <div>
                    {
                        resolveTaskNumber.map(task => <ResolvedTask task={task} key={task.id}></ResolvedTask>)
                    }
                </div>
            }
        </div>
